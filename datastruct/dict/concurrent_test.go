...
func TestConcurrentPutIfAbsent(t *testing.T) {
	...
	for i := 0; i < count; i++ {
		go func(i int) {
			...
			// update
			ret = cvt(d.PutIfAbsent(key, i*10))
			if ret != 0 { // no update
...
func TestConcurrentDelete(t *testing.T) {
	...
	if d.Size() != totalCount {
		t.Error("put test failed: expected len is 100, actual: " + strconv.Itoa(d.Size()))
	}
	for i := 0; i < totalCount; i++ {
		key := "k" + strconv.Itoa(i)
		...
		ret := cvt(d.Delete(key))
		if ret != 1 {
			t.Error("remove test failed: expected result 1, actual: " + strconv.Itoa(ret) + ", key:" + key)
		}
		if d.Size() != totalCount-i-1 {
			t.Error("put test failed: expected len is 99, actual: " + strconv.Itoa(d.Size()))
		}
		_, ok = d.Get(key)
		if ok {
...
func TestConcurrentDict_Keys(t *testing.T) {
	...
	size := 10
	for i := 0; i < size; i++ {
		d.Put(utils.AlnumString(5), utils.AlnumString(5))
	}
	if len(d.Keys()) != size {
